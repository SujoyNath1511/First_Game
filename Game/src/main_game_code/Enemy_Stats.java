...
public class Enemy_Stats {
	...
	public void render (Graphics2D g2d) {
		...
		if (walking == 2) {	
			if (xVelocity == 0) {
				g2d.drawImage(sprite[0], enemyXValue + camera.getXOffset(), enemyYValue + camera.getYOffset(),enemy_width[enemyType-1],enemy_height[enemyType-1], null);//if walking is 2 (moving right)
				
			}
				else
				g2d.drawImage(sprite[frameCounter], enemyXValue + camera.getXOffset(), enemyYValue + camera.getYOffset(),enemy_width[enemyType-1],enemy_height[enemyType-1], null);	//draw the sprite on the screen, based on which part of the walking animation is
			...
			if (frameCounter > 8) { //if frameCounter is greater than 8, reset it to 0 because there are only 9 images for walking animation
				...
				if (xVelocity == 0)
					g2d.drawImage(sprite[9], enemyXValue + camera.getXOffset(), enemyYValue + camera.getYOffset(), enemy_width[enemyType-1],enemy_height[enemyType-1], null);
				else
					g2d.drawImage(sprite[frameCounter + 9], enemyXValue + camera.getXOffset(), enemyYValue + camera.getYOffset(),enemy_width[enemyType-1], enemy_height[enemyType-1], null);
			}
		frameCounter++;
		if (frameCounter > 8) { 
			frameCounter = 0;
	...
	public void bullet_collision(ArrayList <Projectile> projectiles) {
		for (int i = 0; i < projectiles.size(); i++) {
			if (projectiles.get(i).getType() == 1 && projectiles.get(i) != null)
