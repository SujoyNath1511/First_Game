...
public class World {
	...
	public void generateEnemies (Player player, SpriteSheet sheet) {
		...
		enemies[15] = new Enemy_Stats(100,30,3,835,257,camera,player,sheet,2);
		enemies[16] = new Enemy_Stats(100,30,3,930,257,camera,player,sheet,2);
		...
		for (int i = 0; i < enemies.length; i++) {
			if (enemies[i].getHitpoints() > 0) {
				if ((enemies[i].getEnemyXValue() + camera.getXOffset()) <= Frame.WINDOW_WIDTH || 
					enemies[i].getEnemyXValue() + camera.getXOffset() + enemies[i].getEnemyWidth() > 0) {
					enemies[i].render(g);
				}
			}
		}
